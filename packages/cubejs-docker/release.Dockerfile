...
FROM base as build
# Add this line to update TypeScript in the schema-compiler package
# RUN cd packages/cubejs-schema-compiler && yarn add typescript@latest
RUN yarn install
...
# Begin: Copy from latest.Dockerfile
FROM base AS final
...
ENV DEV_BUILD_TAG=2024-12-09_22-20-50
